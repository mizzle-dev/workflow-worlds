...
function generateRegistry() {
  ...
  const imports = files
    .map((file) => {
      ...
      // Use .js extension for Node16/NodeNext module resolution compatibility
      let importPath;
      if (relativeWorkflowsPath && relativeWorkflowsPath !== 'workflows') {
        importPath = `${relativeWorkflowsPath}/${file.replace(/\.ts$/, '.js')}`;
      } else {
        importPath = `./workflows/${file.replace(/\.ts$/, '.js')}`;
      }
